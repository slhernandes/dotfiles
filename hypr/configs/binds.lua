...
hl.bind(MainMod .. "+ E", hl.dsp.exec_cmd(GuiEditor))
hl.bind(MainMod .. "+ F", hl.dsp.exec_cmd(Browser))
-- hl.bind(MainMod .. "+ M", hl.dsp.exec_cmd(ScriptDir .. "/minimize.py minimize"))
hl.bind(MainMod .. "+ M", hl.dsp.exec_cmd("qs ipc call globalStates toggleMinimize false"))
...
hl.bind(MainMod .. "+ N", hl.dsp.exec_cmd("qs ipc call globalStates toggleNotificationCentre"))
-- hl.bind(MainMod .. "+ SHIFT + P", hl.dsp.exec_cmd(ScriptDir .. "/find_pdf"))
hl.bind(MainMod .. "+ SHIFT + P", hl.dsp.exec_cmd("qs ipc call globalStates toggleLauncher middle pdf"))
-- hl.bind(MainMod .. "+ Space", hl.dsp.exec_cmd(ScriptDir .. "/rofisearch.sh"))
hl.bind(MainMod .. "+ Space", hl.dsp.exec_cmd("qs ipc call globalStates toggleLauncher middle websearch"))
...
hl.bind("SHIFT + F7", hl.dsp.exec_cmd("lux -s 5% && qs ipc call brightnessOSD openOSD $(lux -G)"), {repeating = true})
hl.bind("SHIFT + F8", hl.dsp.exec_cmd("lux -a 5% && qs ipc call brightnessOSD openOSD $(lux -G)"), {repeating = true})
...
for i = 1, 9 do
  ...
  hl.bind(MainMod .. "+ SHIFT +" .. ws, function()
    ...
    if activeWindow ~= nil and activeWindow.group ~= nil then hl.dispatch(hl.dsp.window.move({out_of_group = true})) end
...
hl.bind(MainMod .. "+ 0", function()
  ...
  if activeSpecialWs ~= nil then
    local specialWsSuffix = string.match(activeSpecialWs.name, "[^:]+$") or "magic"
...
hl.bind("XF86AudioRaiseVolume", helper.volume("out", "up"), {repeating = true, locked = true})
hl.bind("XF86AudioLowerVolume", helper.volume("out", "down"), {repeating = true, locked = true})
hl.bind("XF86AudioMute", helper.volume("out", "mute"), {locked = true})
hl.bind("XF86AudioMicMute", helper.volume("in", "mute"), {locked = true})
hl.bind("SHIFT + XF86AudioRaiseVolume", helper.volume("in", "up"), {repeating = true, locked = true})
hl.bind("SHIFT + XF86AudioLowerVolume", helper.volume("in", "down"), {repeating = true, locked = true})
...
hl.define_submap("DROPDOWN", "reset", function()
  for _, v in ipairs(specialWs.suffixes) do if v.key ~= nil and v.cmd ~= nil then hl.bind(v.key, helper.namedSp(v)) end end
...
hl.define_submap("PASSTHROUGH", function() hl.bind(MainMod .. "+ escape", hl.dsp.submap("reset")) end)
