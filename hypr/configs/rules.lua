...
hl.window_rule({match = {modal = true}, float = true, move = {"(monitor_w-window_w)/2", "(monitor_h-window_h)/2"}})
...
hl.window_rule({match = {class = "^(com.mitchellh.ghostty)$"}, opacity = "0.95 0.8"})

hl.window_rule({match = {class = "^(Emacs)$"}, opacity = "1.0 0.97", workspace = "2"})

hl.window_rule({match = {class = "^(firefox)$"}, opacity = "1.0 0.97", workspace = "5"})

hl.window_rule({match = {class = "^(floorp)$"}, opacity = "1.0 0.97", workspace = "5"})
...
hl.window_rule({match = {class = "^(steam_app_3513350)$", title = "^(\\s*)$"}, float = true})
...
hl.window_rule({match = {class = "^(gamescope)$"}, workspace = "8", fullscreen = true})

hl.window_rule({match = {class = "^(steam_app_.*)$"}, immediate = true, workspace = "8"})
...
for _, v in ipairs(specialWs.suffixes) do
  hl.workspace_rule({workspace = "name:special:" .. v.name, gaps_out = {top = 4, left = 320, right = 335, bottom = 341}})
end

hl.workspace_rule({workspace = "name:special:minimized", layout = "monocle"})

for i = 2, 5 do hl.workspace_rule({workspace = tostring(i), layout = "scrolling"}) end

hl.layer_rule({match = {namespace = "swaync-.*"}, blur = true, blur_popups = true, ignore_alpha = 0})

hl.layer_rule({match = {namespace = "selection"}, no_anim = true})

hl.layer_rule({match = {namespace = "waybar"}, blur = true, xray = true, ignore_alpha = 0.5})

hl.layer_rule({match = {namespace = "qsBar"}, blur = true, xray = true, ignore_alpha = 0.5})
...
hl.layer_rule({match = {namespace = "rofi"}, blur = true, blur_popups = true, xray = true, ignore_alpha = 0.5})

hl.layer_rule({match = {namespace = "notifications"}, blur = true, blur_popups = true, xray = false, ignore_alpha = 0.5})
